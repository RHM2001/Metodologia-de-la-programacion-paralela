...
double aplicar_ga(const double *d, int n, int n_gen, int tam_pob, int *sol, double m_rate, int rank, int size)
{
	...
	if(rank == 0){
        ...
        int pack_size;
        MPI_Pack_size(isla_poblacion+(isla_poblacion_size*i)+resto, individuo_type, MPI_COMM_WORLD, &pack_size);
        char *packed_data = (char *)malloc(pack_size * size);
        int position = 0;
        for (int i = 0; i < isla_poblacion_size; ++i) {
            MPI_Pack(&isla_poblacion[i], 1, individuo_type, packed_data, pack_size * size, &position, MPI_COMM_WORLD);
        }

        // Enviar datos empaquetados a otras islas
        for (int i = 1; i < size; ++i) {
            MPI_Send(packed_data, position, MPI_PACKED, i, 0, MPI_COMM_WORLD);
        }
		...
		int pack_size;
        MPI_Pack_size(isla_poblacion_size, individuo_type, MPI_COMM_WORLD, &pack_size);
        char *packed_data = (char *)malloc(pack_size);
        MPI_Recv(packed_data, pack_size, MPI_PACKED, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        // Desempaquetar datos en isla_poblacion
        int position = 0;
        for (int i = 0; i < isla_poblacion_size; ++i) {
            MPI_Unpack(packed_data, pack_size, &position, &isla_poblacion[i], 1, individuo_type, MPI_COMM_WORLD);
        }
	...
	for (g = 0; g < n_gen; g++)
	{
		...
		if (NGM == contador_NGM) {
            if (rank == 0) {
                // Empaquetar los mejores individuos
                int pack_size;
                MPI_Pack_size(NEM, individuo_type, MPI_COMM_WORLD, &pack_size);
                char *packed_data = (char *)malloc(pack_size * size);
                int position = 0;
                for (int i = 1; i < size; ++i) {
                    MPI_Pack(isla_poblacion + (isla_poblacion_size * i) + resto, NEM, individuo_type, packed_data, pack_size * size, &position, MPI_COMM_WORLD);
                }

                // Enviar datos empaquetados a las islas
                for (int i = 1; i < size; ++i) {
                    MPI_Send(packed_data, position, MPI_PACKED, i, 0, MPI_COMM_WORLD);
                }
                free(packed_data);

                // Desempaquetar los mejores individuos de las demás islas
                position = 0;
                for (int i = 1; i < size; ++i) {
                    MPI_Recv(packed_data, pack_size * size, MPI_PACKED, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    for (int j = 0; j < NEM; ++j) {
                        MPI_Unpack(packed_data, pack_size * size, &position, isla_poblacion + (isla_poblacion_size * i) + resto + j, 1, individuo_type, MPI_COMM_WORLD);
                    }
                }
                free(packed_data);

                // Combinar y ordenar los mejores individuos
                qsort(isla_poblacion, tam_pob, sizeof(Individuo), comp_fitness);

                // Enviar los mejores individuos a las islas
                for (int i = 1; i < size; ++i) {
                    MPI_Send(isla_poblacion, NEM, individuo_type, i, 0, MPI_COMM_WORLD);
                }
                isla_poblacion_size = isla_poblacion_size + resto;
            } else {
                // Recibir datos empaquetados de la isla 0
                int pack_size;
                MPI_Pack_size(NEM, individuo_type, MPI_COMM_WORLD, &pack_size);
                char *packed_data = (char *)malloc(pack_size);
                MPI_Recv(packed_data, pack_size, MPI_PACKED, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

                // Desempaquetar los mejores individuos
                int position = 0;
                for (int i = 0; i < NEM; ++i) {
                    MPI_Unpack(packed_data, pack_size, &position, isla_poblacion + (isla_poblacion_size * rank) + resto + i, 1, individuo_type, MPI_COMM_WORLD);
                }
                free(packed_data);

                // Enviar los mejores individuos a la isla 0
