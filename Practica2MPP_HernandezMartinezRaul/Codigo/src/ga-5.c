...
double aplicar_ga(const double *d, int n, int n_gen, int tam_pob, int *sol, double m_rate, int rank, int size)
{
	...
	if(rank == 0){
        ...
        rcv_buff = (Individuo *)malloc(tam_pob * sizeof(Individuo));
        assert(rcv_buff);
        ...
        rcv_buff = (Individuo *)malloc(isla_poblacion_size * sizeof(Individuo));
        assert(rcv_buff);
	}

    // Inicialización de la población usando Scatter
    MPI_Scatter(isla_poblacion, isla_poblacion_size, individuo_type, rcv_buff, isla_poblacion_size, individuo_type, 0, MPI_COMM_WORLD);
	...
	for (g = 0; g < n_gen; g++)
	{
		...
		if (NGM == contador_NGM){
            isla_poblacion_size = isla_poblacion_size - resto;
            MPI_Gather(rcv_buff, isla_poblacion_size, individuo_type, isla_poblacion, isla_poblacion_size, individuo_type, 0, MPI_COMM_WORLD);
            ...
            MPI_Bcast(isla_poblacion, tam_pob, individuo_type, 0, MPI_COMM_WORLD);
			isla_poblacion_size = isla_poblacion_size + resto;
