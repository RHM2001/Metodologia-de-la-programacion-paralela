...
double aplicar_ga(const double *d, int n, int n_gen, int tam_pob, int *sol, double m_rate, int rank, int size)
{
	...
	if(rank == 0){
		...
		for(i=1; i < size; i++){
			MPI_Isend(isla_poblacion+(isla_poblacion_size*i)+resto, isla_poblacion_size, individuo_type, i, 0, MPI_COMM_WORLD, &send_request);
		}
		...
		MPI_Irecv(isla_poblacion, isla_poblacion_size, individuo_type, 0, 0, MPI_COMM_WORLD, &recv_request);
		...
		if (NGM == contador_NGM){
			if (rank == 0){
				...
				for(i=1 ; i < size ; i++){
					...
					MPI_Irecv(isla_poblacion + (isla_poblacion_size*i) + resto, NEM, individuo_type, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &recv_request);
                    MPI_Wait(&recv_request, MPI_STATUS_IGNORE);
				}
				...
				for(i=1 ; i < size ; i++){
					...
					MPI_Isend(isla_poblacion, NEM, individuo_type, i, 0, MPI_COMM_WORLD, &send_request);
				}
				isla_poblacion_size = isla_poblacion_size + resto;
			}
			else{
				// Se envian a la isla 0 los mejores individuos
				MPI_Isend(isla_poblacion, NEM, individuo_type, 0, 0, MPI_COMM_WORLD, &send_request);
				// Se reciben los mejores individuos que hay
				MPI_Irecv(isla_poblacion, NEM, individuo_type, 0, 0, MPI_COMM_WORLD, &recv_request);
                MPI_Wait(&recv_request, MPI_STATUS_IGNORE);
			}
			contador_NGM = 0;
		}
